...
from imagenet_utils import decode_predictions, decode_imagenet_label
from imagenet_utils import preprocess_input
from keras.preprocessing import image as image_utils

from kyu.layers import get_custom_objects
from kyu.visualize.vis_utils import show_top_k_cam, get_top_k_classification
...
class MyApp(QtGui.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # add my own stuff
        self.displayCAM.clicked.connect(self.displayClassActivationMap)
    ...
    def browseModel(self):
        ...
        self.model = load_model(self.fname, custom_objects=get_custom_objects())
        self.display_model = load_model(self.fname, custom_objects=get_custom_objects())
        print('-----------Loaded successfully---------------------')
        # ctypes.windll.user32.MessageBoxA(0, "Model Loaded Successfuly", "Message", 1)
        messageBox = QtGui.QMessageBox.about(self, "MessageBox", "Model Loaded Successfully")
    ...
    def layerConfiguration(self):
        ...
        featuremap_size = output_shape[1:3]
        num_of_featuremaps = output_shape[3]
    ...
    def browseInputImage(self):
        ...
        # TODO Keep a copy, make changes
        self.original_image = image_utils.load_img(self.imageName, target_size=(224, 224))
        self.original_image = image_utils.img_to_array(self.original_image)
    ...
    def displayClassActivationMap(self):
        print("Display CAM")
        ...
        model = utils.apply_modifications(model, custom_objects=get_custom_objects())
        # TODO Fix logic later
        penultimate_layer_idx = None
        result = get_top_k_classification(self.model, self.image, k=5)
        print("Result: {}".format(result))
        modifier = None
        label = np.asanyarray(1000*[0,])
        label[self.pred_label] = 1
        # label = np.expand_dims(label, 0)
        plt = show_top_k_cam(model, layer_idx, result, 5, self.image, self.original_image, label,
                             decode_fn=decode_imagenet_label,
                             modifier=modifier, title='ImageNet')
        file_name = 'top_k_cam.png'
        plt.savefig(file_name)
        cam_figure = QtGui.QPixmap(file_name)
        self.displayOutput.setScaledContents(True)
        self.displayOutput.setPixmap(cam_figure)
