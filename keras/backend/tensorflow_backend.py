...
def batch_dot(x, y, axes=None):
    ...
    if ndim(x) == 2 and ndim(y) == 2:
        ...
        if hasattr(tf, 'batch_matmul'):
            try:
                out = tf.matmul(x, y, adj_a=adj_x, adj_b=adj_y)
            except TypeError:
                out = tf.matmul(x, y, adj_x=adj_x, adj_y=adj_y)
        else:
...
def svd(x, full_matrices=False, compute_uv=True):
    """
    Add tensorflow svd into keras implementation.
    ...
    """
    return tf.svd(x, full_matrices, compute_uv)
