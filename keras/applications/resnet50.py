...
def identity_block(input_tensor, kernel_size, filters, stage, block):
    ...
    if K.backend() == 'tensorflow':
        ...
        with tf.name_scope('identity_block_{}_{}'.format(stage, block)):
        ...
        raise RuntimeError("Not theano anymore. ")
    return x
...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2)):
    ...
    if K.backend() == 'tensorflow':
        import tensorflow as tf
        with tf.name_scope('conv_block_{}_{}'.format(stage, block)):
...
def ResCovNet50(include_top=True, weights='imagenet',
                input_tensor=None, input_shape=None,
                last_avg=True):
    # Determine proper input shape
    input_shape = _obtain_input_shape(input_shape,
    ...
    if input_tensor is not None:
        ...
        inputs = img_input
    # Create model.
    model = Model(inputs, [block1_x, block2_x, block3_x], name='resnet50')
