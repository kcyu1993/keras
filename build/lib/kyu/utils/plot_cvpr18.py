...
def comparision_pv_convergence_dual():
    ...
    xlabel = 'Num Epochs'
    ylabel = 'Loss'
    filename = '/Users/kcyu/mount/plots/convergence-all.eps'



    # Define the subplot accordingly.
    fig, (ax1, ax2) = plt.subplots(1, 2, sharey=False, sharex=False, figsize=(10, 7))
    ...
    for ind, lplt in enumerate((ax1, ax2)):
        ...
        lplt.set_xlim(xlim)
        lplt.grid(True, axis='y', ls='dotted', lw=.2, c='k', alpha=1)
...
def data_cross_pv():
    DTD = [0.6556, 0.68187, 0.68981, 0.69006, 0.68982, 0.69067, 0.69206, 0.67891, 0.67348]
    MINC = [0.71,0.74187, 0.73557, 0.75096, 0.76154, 0.77012, 0.77999, 0.76503, 0.73012]
    MIT = [0.72,0.75378, 0.7613, 0.7694, 0.76946, 0.7875, 0.7945, 0.79381, 0.76964]

    list_y = [DTD, MINC, MIT]
    list_x = [a for a in range(5, 14)]
    labels = ['DTD', 'MINC', 'MIT']
...
def cross_validate_pv_subplot():
    ...
    plt-path
    """
    filename = '/Users/kcyu/mount/plots/pv_cross-sep-v2.eps'
    list_x, list_y, labels = data_cross_pv()
    ...
    y_lims = [[0.65,0.72], [0.7,0.8], [0.69,0.84]]
    x_lim = [4, 14]
    # CBP-TS baseline
    ...
    for ind, (d, wv) in enumerate(zip(list_y, list_x)):
        ...
        # axes[ind].grid(True, axis='y', ls='dotted', lw=.2, c='k', alpha=1)
        axes[ind].set_ylim(y_lims[ind])

        # legend
        leg = axes[ind].legend(loc=2,shadow=False)
        leg.draw_frame(True)
        ltext = leg.get_texts()
        # axes[ind].setp(ltext, fontsize=18)

    plt.xlabel('PV dimension log(p)')
    plt.xlim(x_lim)
    # Common y label
    fig.text(0.01, 0.5, 'Top 1 accuracy', va='center', rotation='vertical')

    # fig.text(0.90, 0.75, data_label[0], va='center', rotation=270)
